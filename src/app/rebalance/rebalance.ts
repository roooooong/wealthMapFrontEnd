...
export class Rebalance implements OnInit {
  ...
  loadPortfolioFromDb() {
    ...
    this.httpClientService.getApi(`https://wealthmapbackend-production-85e8.up.railway.app/api/rebalance/list/${this.userId}`).subscribe((res: any) => {
      if (res && Array.isArray(res)) {
        ...
        if (this.portfolio) {
          ...
          this.portfolio.forEach(s => {
            this.httpClientService.getApi(`https://wealthmapbackend-production-85e8.up.railway.app/api/strategy-set/quote/${s.stockId}`)
              .subscribe({
                next: (res: any) => {
  ...
  addAsset() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.stockId) {
        ...
        this.httpClientService.postApi('https://wealthmapbackend-production-85e8.up.railway.app/api/rebalance/save', payload).subscribe({
          next: (savedItem: any) => {
            // 存檔成功後，將資料壓入畫面陣列
            // this.portfolio.push({
            //   id: savedItem.id,
  ...
  onDelete(index: number) {
    ...
    if (confirm(`確定要移除 ${item.stockId} 嗎？`)) {
      if (item.id) {
        this.httpClientService.delApi(`https://wealthmapbackend-production-85e8.up.railway.app/api/rebalance/delete/${item.id}`).subscribe(() => {
