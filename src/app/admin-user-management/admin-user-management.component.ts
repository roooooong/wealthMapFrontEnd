...
export class AdminUserManagementComponent {
  ...
  fetchUsers(){
    this.httpClientService.getApi(`https://wealthmapbackend-production-85e8.up.railway.app/api/auth/user-list`)
    .subscribe((res: any) => {
  ...
  showDialog(no:number,user:UserAdminViewDTO,actionText:string) {
    ...
    dialogRef.afterClosed().subscribe((isConfirm) => {
      ...
      //如果有值傳遞出來
      if (isConfirm) {
        console.log(`https://wealthmapbackend-production-85e8.up.railway.app/api/auth/${user.id}/enabled`);
        this.httpClientService.patchApi(`https://wealthmapbackend-production-85e8.up.railway.app/api/auth/${user.id}/enabled`, {})
          .subscribe({
            next: () => {
