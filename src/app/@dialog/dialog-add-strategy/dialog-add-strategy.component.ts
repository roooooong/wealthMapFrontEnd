...
export class DialogAddStrategyComponent {
  ...
  ngOnInit(): void {
    ...
    this.exampleService.user$.subscribe(user => {
      if (user && user.id !== 0) {
        this.userId = user.id;
        this.httpClientService.getApi(`https://wealthmapbackend-production-5c68.up.railway.app/api/strategy-set/user/available-stocks/${this.userId}`)
          .subscribe((res: any) => {
            if (!res) return;
  ...
  onStockChange() {
    ...
    this.httpClientService.getApi(`https://wealthmapbackend-production-5c68.up.railway.app/api/strategy-set/quote/${symbol}`)
      .subscribe((res: any) => {
        console.log(res);
        if (res.code === 200) {
          // 更新畫面上的唯讀欄位
  ...
  confirm() {
    ...
    // 這裡執行 API 更新邏輯
    this.httpClientService.postApi(`https://wealthmapbackend-production-5c68.up.railway.app/api/strategy-set/user/${this.userId}`, this.newStrategy)
      .subscribe((res: any) => {
        if (res.code === 200) {
