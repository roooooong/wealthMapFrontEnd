...
export class DialogAddRebalanceComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.exampleService.user$.subscribe(user => {
      if (user && user.id !== 0) {
        ...
        const apiUrl = `https://wealthmapbackend-production-412c.up.railway.app/api/assets/rebalance/available-stocks/${this.userId}`;
    ...
    const apiUrl = `https://wealthmapbackend-production-412c.up.railway.app/api/assets/user/${this.userId}`;
  }
  ...
  onStockChange() {
    ...
    if(this.assets.some(a => a.type === 'STOCK' && a.symbol === this.newAsset.stockId )){
      ...
      // }
      this.isLoading = true;
      this.httpClientService.getApi(`http://wealthmapbackend-production-412c.up.railway.app/api/strategy-set/quote/${this.newAsset.stockId}`)
      .subscribe((res: any) => {
        if (res.code === 200) {
    ...
    // const quoteUrl = `https://wealthmapbackend-production-412c.up.railway.app/api/strategy-set/quote/${this.newAsset.stockId}`;
    // // 獲取即時報價
    // this.httpClientService.getApi(`https://wealthmapbackend-production-412c.up.railway.app/api/strategy-set/quote/${this.newAsset.stockId}`)
    //   .subscribe((res: any) => {
    //     if (res.code === 200) {
    //       this.currentPrice = res.data.currentPrice;
    //     }
