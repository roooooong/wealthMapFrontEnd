...
export class StrategyListComponent {
  ...
  saveEdit(index: number) {
    ...
    // 這裡執行 API 更新邏輯
    this.httpClientService.putApi(`https://wealthmapbackend-production-412c.up.railway.app/api/strategy-set/${updatedStrategy.id}`,updatedStrategy)
    .subscribe((res:any) => {
      console.log(res);
      if (res && res.code === 200) {
  ...
  onDelete(index: number){
    ...
    this.httpClientService.delApi(`https://wealthmapbackend-production-412c.up.railway.app/api/strategy-set/${this.strategies[index].id}`)
    .subscribe((res:any) => {
      if (res.code === 200) {
        // 畫面移除這張卡片
  ...
  fetchStrategies(userId:number){
    ...
    this.strategies.forEach(s => {
      this.httpClientService.getApi(`https://wealthmapbackend-production-412c.up.railway.app/api/strategy-set/quote/${s.symbol}`)
        .subscribe({
          next: (res: any) => {
