...
refreshChart(data: any[]) {
  ...
  ngOnInit() {
    ...
    this.exampleService.user$.subscribe(user => {
        if (user && user.id && user.id !== 0) {
          ...
          if (this.role === 'USER' || this.role === 'ADMIN') {
            //同步使用者總資產(用於折線圖)
            this.httpClientService.postApi(`https://wealthmapbackend-production-412c.up.railway.app/api/asset-history/sync/${this.userId}`)
              .subscribe((totalasset: any) => {
                console.log('同步userid=', this.userId, '的資產');

                //取得使用者總資產(用於折線圖)
                this.httpClientService.getApi(`https://wealthmapbackend-production-412c.up.railway.app/api/asset-history/${this.userId}`)
    ...
    // 取得前台新聞列表
    this.httpClientService.getApi(`https://wealthmapbackend-production-412c.up.railway.app/api/news/user/list`)
      .subscribe((news: any) => {
