...
export class AssetService {
    ...
    private apiUrl = 'https://wealthmapbackend-production-85e8.up.railway.app/api/assets';
    ...
    updateAsset(id: number, payload: any): Observable<any> {
        ...
        // 如果你有宣告 baseUrl (例如 this.apiUrl = 'https://wealthmapbackend-production-85e8.up.railway.app/api/assets')
        // 那就寫成 return this.http.put(`${this.apiUrl}/${id}`, payload);

        // 如果你是直接寫死網址，就用下面這行：
        return this.http.put(`https://wealthmapbackend-production-85e8.up.railway.app/api/assets/${id}`, payload);
    }
    ...
    searchStock(symbol: string): Observable<any> {
        ...
        return this.http.get<any>(`https://wealthmapbackend-production-85e8.up.railway.app/api/assets/search-stock/${symbol}`).pipe(
            tap(res => {
